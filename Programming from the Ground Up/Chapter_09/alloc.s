...
#######CONSTANTS######## 
 .equ UNAVAILABLE, 0 		#This number will be used to mark
 							#space that has been given out
 .equ AVAILABLE, 1			#This number will be used to mark
...
allocate_init:
 ...
 #				for a new one.
 #
 #PARAMETERS:	The function has one parameter - the size
...
alloc_loop_begin:				#here we iterate through each
 ...
 #If the space is unavailable, go to then next one
 cmpl	$UNAVAILABLE, HDR_AVAIL_OFFSET(%eax)
 je		next_location

 cmpl	%edx, %ecx				#If the space is available, compare
 je		allocate_here			#the size to the needed size. If it's
...
allocate_here:					#if we've made it here,
								#that means that the region header
								#of the region to allocate
								#is in %eax
...
deallocate:
 ...
 #(normally this is 8(%esp), but since
 #we didn't push %ebp or move %esp to 
 #%ebp, we can just do 4(%esp))
 movl	ST_MEMORY_SEG(%esp), %eax

 #get the pointer to the real beginning of memory
